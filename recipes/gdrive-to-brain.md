...
- Pace gateway/API calls and back off on rate-limit responses instead of failing the whole sync immediately
...
### Rate Limits and Large Roots

The collector should assume Drive and gateway calls may be rate-limited, especially on first sync or when the configured root is large.

- Pace discovery/export requests instead of firing them as fast as possible
- On rate-limit responses, retry with exponential backoff and jitter
- Prefer a specific Drive folder as the configured root instead of all of My Drive when possible
- If the initial sync can be long-running, resumable discovery is a good follow-up improvement
